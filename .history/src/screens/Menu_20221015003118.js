...
const loc = ['location1', 'location2', 'location3', 'location4', 'location5'];

const Menu = props => {
  // console.log(props,"haiiiii09090")
  const dispatch = useDispatch();
  const [processArray, setProcessArray] = useState(
    useSelector(state => state.ProcessAndEquipmentReducer?.ProcessLineData),
  );
  ...
  return (
    <>
          ...
          {tab === 'Process Line' ? (
            ProcessData !== undefined && ProcessData.length > 0 ? (
              <View
                style={{
                  marginBottom:
                    Platform.OS == 'ios' ? heightToDp('52') : heightToDp('60'),
                }}>
                <FlatList
                  bounces={false}
      ...
      /> */}
            ...
            {customList.map(i => (
              <TouchableOpacity
                onPress={() => selectedItem(i)}
                style={{
                  padding: 10,
                  backgroundColor: i.checked ? 'black' : 'yellow',
                  width: widthToDp('26'),
                  alignItems: 'center',
